...
sphere_material = Material(
    color=[0.9, 0.2, 0.2], roughness=1, metallic=0.0, specularity=0.0,
    em_strength=0.0, em_color=[0, 0, 0], ir=1.0
)
sphere_material1 = Material(
    color=[0.2, 0.7, 0.9], roughness=1, metallic=0.0, specularity=0.0,
    em_strength=0.0, em_color=[0, 0, 0], ir=1.0
)

emmision_sphere = Material(
    color=[0,0,0],roughness=1, metallic=0,specularity=0,em_color=[1,1,1],em_strength=10,ir=1
)
...
sphere = Object.Sphere(
    centers=torch.tensor([
        ...
        [0.0, -999.0, 0.0],
        [10,10,10]     # big ground sphere
    ], device='cuda'),
    radii=torch.tensor([1.0, 998.0,10], device='cuda'),
    materials=[sphere_material, sphere_material1,emmision_sphere]
)

light = Illumination.Light(
    center=[20.0, 30.0, 20.0], radius=20, color=[1.0, 1.0, 1.0], strength=1.0
)
...
def render(scene, camera, sampler, width, height, num_samples=64, max_bounces=5, AA=True):
    accum_buffer = torch.zeros((height, width, 3), device='cuda')
    
    for sample in range(num_samples):
        # Generate initial rays
        ray_batch = camera.CreateRayBffr(width, height, 'persp', sampler, sample, AA)
        color_buffer = torch.zeros_like(ray_batch.origins, device='cuda')
        active_mask = torch.ones(ray_batch.origins.shape[0], dtype=torch.bool, device='cuda')
        attenuation = torch.ones_like(ray_batch.origins, device='cuda')
        
        current_rays = ray_batch
        for bounce in range(max_bounces + 1):
            hit_info = scene.interact(current_rays)
            miss_mask = (hit_info.t == float('inf'))
            hit_mask = ~miss_mask & active_mask  # ✅ Fixed hit_mask
            
            # Handle misses
            color_buffer[miss_mask] += scene.background_color * attenuation[miss_mask]
            
            if not hit_mask.any():
                break  # Early exit
            
            # Process hits
            hit_points = current_rays.origins[hit_mask] + current_rays.directions[hit_mask] * hit_info.t[hit_mask].unsqueeze(-1)
            hit_normals = hit_info.normal[hit_mask]
            mat_idx = hit_info.material_idx[hit_mask]
            
            # Material properties
            materials = scene.objects.materials
            mat_colors = torch.stack([m.color for m in materials], 0).to('cuda')[mat_idx]
            mat_emission = torch.stack([m.em_color * m.em_strength for m in materials], 0).to('cuda')[mat_idx]
            
            # Add emission
            color_buffer[hit_mask] += mat_emission * attenuation[hit_mask]
            
            # --- Direct illumination (vectorized) ---
            if bounce == 0:
                for light in scene.lights:
                    to_light = light.center - hit_points
                    dist = torch.norm(to_light, dim=-1, keepdim=True)
                    light_dir = to_light / (dist + 1e-8)
                    
                    shadow_origins = hit_points + 1e-4 * hit_normals
                    shadow_rays = RayBatch(shadow_origins, light_dir)
                    shadow_hits = scene.interact(shadow_rays)
                    
                    visible = (shadow_hits.t > dist.squeeze(-1) - 1e-3)
                    lambert = torch.clamp((hit_normals * light_dir).sum(-1, keepdim=True), 0.0)
                    
                    direct = visible.unsqueeze(-1) * light.color * light.strength * lambert * mat_colors
                    color_buffer[hit_mask] += direct * attenuation[hit_mask]

            # Prepare next bounce
            attenuation[hit_mask] *= mat_colors
            
            # Russian Roulette termination
            survival_prob = torch.min(attenuation[hit_mask].mean(-1) * 0.8 + 0.2, torch.tensor(1.0))
            survival_mask = torch.rand_like(survival_prob) < survival_prob
            
            # Update active_mask correctly
            new_active_mask = torch.zeros_like(active_mask)
            new_active_mask[hit_mask] = survival_mask  # ✅ Correct mapping
            active_mask = new_active_mask
            
            # Generate new rays
            new_dirs = Lambertian.offset(
                hit_normals[survival_mask],
                torch.tensor([m.roughness for m in materials], device='cuda')[mat_idx[survival_mask]],
                current_rays.directions[hit_mask][survival_mask]
            )
            current_rays = RayBatch(
                hit_points[survival_mask] + 1e-4 * hit_normals[survival_mask],
                new_dirs,
                device='cuda'
            )
        
        # Accumulate sample
        accum_buffer += color_buffer.reshape(height, width, 3)
    
    return (accum_buffer / num_samples).clamp(0, 1)


render(scene=scene, camera=camera, sampler=sampler, width=width, height=height)